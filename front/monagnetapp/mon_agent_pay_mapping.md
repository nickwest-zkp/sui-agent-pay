...
| Dashboard header | Connected address, network, wallet state | `@mysten/dapp-kit-react` hooks | Replaces the old EVM wallet stack. |
...
| Review agent state | Dashboard client | `/api/agents` and Move registry objects | Reads the Sui registry instead of EVM contract storage. |
...
- Do not add EVM-specific hooks, contract artifacts, or provider config back into this subtree.
